// modifications by _wb_ and notaz to make this faster on the Pandora
/*
...
*/
inline int fastPixelColorNolock(SDL_Surface * dst, Sint16 x, Sint16 y, Uint32 color)
{
...
\returns Returns 0 on success, -1 on failure.
*/
inline int fastPixelColorNolockNoclip(SDL_Surface * dst, Sint16 x, Sint16 y, Uint32 color)
{
...
\returns Returns 0 on success, -1 on failure.
*/
inline int fastPixelColor(SDL_Surface * dst, Sint16 x, Sint16 y, Uint32 color)
{
	int result;
...
\returns Returns 0 on success, -1 on failure.
*/
inline int fastPixelRGBA(SDL_Surface * dst, Sint16 x, Sint16 y, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	Uint32 color;
...
\returns Returns 0 on success, -1 on failure.
*/
inline int fastPixelRGBANolock(SDL_Surface * dst, Sint16 x, Sint16 y, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	Uint32 color;
...
\returns Returns 0 on success, -1 on failure.
*/
inline int _putPixelAlphaNoclip(SDL_Surface *dst, Sint16 x, Sint16 y, Uint32 color, Uint8 alpha)
{
...
inline int _putPixelAlpha(SDL_Surface *dst, Sint16 x, Sint16 y, Uint32 color, Uint8 alpha)
{
//	SDL_PixelFormat *format;
...
*/
	...
	{
	    return _putPixelAlphaNoclip(dst, x, y, color, alpha);
	}
	return (0);
}
...
\returns Returns 0 on success, -1 on failure.
*/
inline int pixelColor(SDL_Surface * dst, Sint16 x, Sint16 y, Uint32 color)
{
	Uint8 alpha;
	Uint32 mcolor;
	int result = 0;
...
inline int pixelColorNolockNoclip(SDL_Surface * dst, Sint16 x, Sint16 y, Uint32 color)
{
	Uint8 alpha;
	Uint32 mcolor;
	int result = 0;
	...
	* Draw 
	*/
	result = _putPixelAlphaNoclip(dst, x, y, mcolor, alpha);
...
\returns Returns 0 on success, -1 on failure.
*/
inline int _filledRectAlpha(SDL_Surface * dst, Sint16 x1, Sint16 y1, Sint16 x2, Sint16 y2, Uint32 color, Uint8 alpha)
{
//	SDL_PixelFormat *format;
...
/*			const Uint32 Rmask = 16711680;
			...
			const Uint32 dR = (color & Rmask);
			const Uint32 dG = (color & Gmask);
			const Uint32 dB = (color & Bmask);
			const Uint32 dA = (color & Amask);*/

			if (!color) {
			  // makes nearly no difference (about 0.5%)
			  alpha = 255-alpha;
			  for (y = y1; y <= y2; y++) {
				extern void draw_hline_black_alpha_neon(Uint32 *dst, int count, Uint32 alpha);
				row = (Uint32 *) dst->pixels + y * dst->pitch / 4;
				draw_hline_black_alpha_neon(row + x1, x2 - x1 + 1, alpha);
			  }
			} else {
			// check for a freak border case that could cause memory corruption or segfault,
			// and do it in the old way for the last line
			if (y2 == dst->clip_rect.y+dst->clip_rect.h-1) {
				row = (Uint32 *) dst->pixels + y2 * dst->pitch / 4;
	register int x;
			register Uint32 *pixel;
			const Uint32 Rmask = 16711680;
			const Uint32 Gmask = 65280;
			...
			for (y = y1; y <= y2; y++) {
				extern void draw_hline_alpha_neon(Uint32 *dst, int count, Uint32 color, Uint32 alpha);

				row = (Uint32 *) dst->pixels + y * dst->pitch / 4;
#if 1
				draw_hline_alpha_neon(row + x1, x2 - x1 + 1, color, alpha);
#else
				for (x = x1; x <= x2; x++) {
					pixel = row + x;
					register Uint32 R, G, B;
...
\returns Returns 0 on success, -1 on failure.
*/
inline int filledRectAlpha(SDL_Surface * dst, Sint16 x1, Sint16 y1, Sint16 x2, Sint16 y2, Uint32 color)
{
	Uint8 alpha;
	Uint32 mcolor;
	//int result = 0;
	...
	*/
	//result = 
	_filledRectAlpha(dst, x1, y1, x2, y2, mcolor, alpha);
...
\returns Returns 0 on success, -1 on failure.
*/
inline int _HLineAlpha(SDL_Surface * dst, Sint16 x1, Sint16 x2, Sint16 y, Uint32 color)
{
...
\returns Returns 0 on success, -1 on failure.
*/
inline int pixelColorWeight(SDL_Surface * dst, Sint16 x, Sint16 y, Uint32 color, Uint32 weight)
{
	Uint32 a;
...
inline int pixelColorWeightNolockNoclip(SDL_Surface * dst, Sint16 x, Sint16 y, Uint32 color, Uint32 weight)
{
	Uint32 a;
	...
	*/
	...
	return (pixelColorNolockNoclip(dst, x, y, (color & (Uint32) 0xffffff00) | (Uint32) a));
}
...
\returns Returns 0 on success, -1 on failure.
*/
inline int pixelRGBA(SDL_Surface * dst, Sint16 x, Sint16 y, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	Uint32 color;
...
\returns Returns 0 on success, -1 on failure.
*/
inline int hlineColorStore(SDL_Surface * dst, Sint16 x1, Sint16 x2, Sint16 y, Uint32 color)
{
...
\returns Returns 0 on success, -1 on failure.
*/
inline int hlineRGBAStore(SDL_Surface * dst, Sint16 x1, Sint16 x2, Sint16 y, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	/*
	* Draw 
	*/
...
//	int result = -1;
	Uint8 *colorptr;
		...
		* Alpha blending blit 
		*/
		  return _HLineAlpha(dst, x1, x1 + dx, y, color);
	}
	...
	*/
	if ((color & 255) == 255) {
		...
		*/
		...
		re.w = w+1;
		re.h = h+1;
		SDL_FillRect(dst,&re,SDL_MapRGB(dst->format, colorptr[3], colorptr[2], colorptr[1]));
#else
	...
	if ((color & 255) == 255) {
		...
		*/
		...
		if (ax > ay) {
			...
			while (x != x2) {
				//pixelColorNolockNoclip (dst, x, y, color);
				_putPixelAlphaNoclip(dst, x, y, mcolor, alpha);
				if (d > 0 || (d == 0 && sx == 1)) {
					y += sy;
					d -= ax;
				}
...
\returns Returns 0 on success, -1 on failure.
*/
inline int _aalineColor(SDL_Surface * dst, Sint16 x1, Sint16 y1, Sint16 x2, Sint16 y2, Uint32 color, int draw_endpoint)
{
	...
	if (dx == 0) {
		...
		*/
		if (draw_endpoint)
		{
			...
			if (dy>0) {
				return (vlineColor(dst, x1, y1, y2+(y2>y1?-1:1), color));
			} else {
				return (pixelColor(dst, x1, y1, color));
			}
		}
		...
		*/
		if (draw_endpoint)
		{
			...
			if (dx>0) {
				return (hlineColor(dst, xx0, xx0+dx-1, y1, color));
			} else if (dx<0) {
				return (hlineColor(dst, xx0+dx+1, xx0, y1, color));
			} else {
				return (pixelColor(dst, x1, y1, color));
			}
		}
	...
	*/
	result |= pixelColorNolockNoclip(dst, x1, y1, color);
		...
		*/
		...
		while (--dy) {
			...
			*/
			...
			result |= pixelColorWeightNolockNoclip (dst, xx0, yy0, color, 255 - wgt);
			result |= pixelColorWeightNolockNoclip (dst, x0pxdir, yy0, color, wgt);
		}
		...
		*/
		...
		while (--dx) {
			...
			*/
			wgt = (erracc >> intshift) & 255;
			result |= pixelColorWeightNolockNoclip (dst, xx0, yy0, color, 255 - wgt);
			result |= pixelColorWeightNolockNoclip (dst, xx0, y0p1, color, wgt);
		}
	}
	...
	*/
	if (draw_endpoint) {
		...
		* need to be weighted. 
		*/
		result |= pixelColorNolockNoclip (dst, x2, y2, color);
	}
...
int aaellipseColor(SDL_Surface * dst, Sint16 x, Sint16 y, Sint16 rx, Sint16 ry, Uint32 color)
{
	...
	*/
	...
	dxt = (Sint16)lrint((float)a2 / sab) + od;
	...
	dyt = (Sint16)lrint((float)b2 / sab ) + od;    
...
int _pieColor(SDL_Surface * dst, Sint16 x, Sint16 y, Sint16 rad, Sint16 start, Sint16 end, Uint32 color, Uint8 filled)
{
	...
	Sint16 x1, y1, x2, y2;
	int result;
	float angle, start_angle, end_angle;
	float deltaAngle;
	float dr;
	int numpoints, i;
	Sint16 *vx, *vy;
...
//	deltaAngle = 1.0;
	start_angle = (float) start *(2.0 * M_PI / 360.0);
	end_angle = (float) end *(2.0 * M_PI / 360.0);
	if (start > end) {
		end_angle += (2.0 * M_PI);
	}
...
*/
float _evaluateBezier (float *data, int ndata, float t) 
{
	float mu, result;
	int n,k,kn,nn,nkn;
...
int bezierColor(SDL_Surface * dst, const Sint16 * vx, const Sint16 * vy, int n, int s, Uint32 color)
{
	...
	* Variable setup 
	*/
	stepsize=(float)1.0/(float)s;

	/* Transfer vertices into float arrays */
	if ((x=(float *)malloc(sizeof(float)*(n+1)))==NULL) {
		return(-1);
	}
	if ((y=(float *)malloc(sizeof(float)*(n+1)))==NULL) {
		free(x);
		return(-1);
	}    
	for (i=0; i<n; i++) {
...
#define HYPOT(x,y) sqrt((float)(x)*(float)(x)+(float)(y)*(float)(y)) 
...
{	
	...
	ml1x= ml1y = ml2x = ml2y = ml1bx = ml1by = ml2bx = ml2by = 0;
	...
	ang = atan((float) m->v / (float) m->u);	/* calc new initial point - offset both sides of ideal */	
	sang = sin(ang);
	cang = cos(ang);
